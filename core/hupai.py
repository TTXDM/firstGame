...
def isHu(_pai):
    ...
    try:
        ...
        if not re.match(r'^[1-9A-Ia-irstuxyz]{14,}$', _pai):
            raise Exception('牌型错误.')

        if len(_pai) == 14 and re.compile(r'(?:(\w)\1{1}){7}').match(_pai):
            raise HasHuPai('七对')

        if re.sub(r'(\w)\1', '\\1', _pai) == '19AIairstuxyz':
            raise HasHuPai('国士无双')

        _sort = [ re.findall(_r, _pai) for _r in [r'\d', r'[A-I]', r'[a-i]', r'[rstuxyz]']]
...
if __name__ == "__main__":
    ...
    #s = isHu('19AIairstuxyzm')
    #s = isHu('Gi4b5HIwgbwhb6')
    # s = isHu('1231ccc2322abc')

    print isHu("456CDEbbbcdett")
